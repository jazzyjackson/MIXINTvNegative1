...
async function buildTemplateArray(guigraph){
     ...
     * So this recurses over the progency properties, opens files if they exist,
    ...
    await guigraph.asyncForEach(async function pushTemplates(guiblock){
        let [ tagName, attrObj ] = Object.entires(guiblock)[0]
        let prefix = path.join(appRoot, 'gui-blocks', tagName)
        ...
        if(Array.isArray(attrObj.mixins)){
            attrObj.mixins.asyncForEach(async filepath => {
                ...
                let mixin = await getReadStreamOrNull(path.join(appRoot, 'gui-mixins', filepath))
        ...
        // Check that progency property exists + is an Array 
        if(Array.isArray(attrObj.progency)){
            // and recursively call this function to append all the templates in order
            await attrObj.progency.asyncForEach(pushTemplates)
        }
    })
